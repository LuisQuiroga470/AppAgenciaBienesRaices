...
class GestionAgentes : AppCompatActivity() {
    ...
    private val campos = listOf("Nombre", "RUT", "Teléfono", "Correo", "Comisión (%)", "Estado")
    private val numeros = listOf("Comisión (%)")
    private lateinit var lista: LinearLayout

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_gestion_agentes)
    ...
    private fun mostrarRegistros() {
        ...
        for (registro in registros) {
            val tarjeta = layoutInflater.inflate(R.layout.tarjeta_agentes, lista, false)
    ...
    private fun abrirFormulario(registro: Registro?) {
        val formulario = layoutInflater.inflate(R.layout.formulario_agentes, null)
        val entradas = listOf<View>(
            ...
            formulario.findViewById(R.id.comision),
        ...
        formulario.findViewById<Button>(R.id.btnGuardar).setOnClickListener {
            ...
            for ((posicion, entrada) in entradas.withIndex()) {
                ...
                if (valor.isBlank()) {
                    ...
                    } else if (campo == "Comisión (%)" && numero > 100) {
            ...
            if (valido) {
                if (registro == null) {
                    registros.add(Registro(Datos.siguienteAgente++, valores))
                } else {
